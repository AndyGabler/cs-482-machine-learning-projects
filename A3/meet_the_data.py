...
required meta-data from the Machine data. 
...
data, targets, feature_names, target_names, file_name = csv.read_csv("machine.data")
...
print("Description of data: Machine data of size", str(data.shape[0]), "from file", file_name)
...
data_frame = pd.read_csv("machine.data")
# First two columns are make-model, aka indices. we don't need these
data_frame = data_frame.drop([data_frame.columns[0], data_frame.columns[1]], axis=1)
